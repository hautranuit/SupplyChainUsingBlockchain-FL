...
def generate_features_and_label_for_batch_monitoring(node_address_checksum, all_events):
    ...
    if label_val == 1:
        print(f"  Node {node_address_checksum} labeled ANOMALOUS for Batch Monitoring (score: {anomaly_score}). F:[{features[0]:.0f},{features[1]:.2f},{features[2]:.0f},{features[3]:.2f},{features[4]:.1f},{features[5]:.2f}]")
    return features, label

def load_real_data_for_fl_client_batch_monitoring(client_id: str, target_node_addresses: list[str], bc_connector):
    ...
    for node_addr_orig in target_node_addresses:
        try:
            ...
            features, label = generate_features_and_label_for_batch_monitoring(node_addr_checksum, EVENT_CACHE)
...
def make_federated_data_batch_monitoring_real(all_target_node_addresses: list[str], num_fl_clients: int):
    ...
    for i in range(num_fl_clients):
        ...
        if not client_node_list:
            ...
            client_ds = load_real_data_for_fl_client_batch_monitoring(client_id, client_node_list, bc_connector)
...
if __name__ == '__main__':
    ...
    num_clients_test = 1 
    federated_train_data_test = make_federated_data_batch_monitoring_real(example_node_addresses, num_fl_clients=num_clients_test)
