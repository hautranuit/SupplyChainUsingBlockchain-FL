...
describe("Supply Chain Cost Analysis (Setup & Marketplace Only)", function () {
    let supplyChainNFT, owner, manufacturer, retailer, sn1, pn1, pn2, pn3, buyer; // Required signers
    let signers;
    let tokenId;
    ...
    const results = {
        ...
        initiatePurchase: BigInt(0), // Added
        depositCollateral: BigInt(0), // Added (includes NFT transfer)
    ...
    before(async function () {
        ...
        if (signers.length < 7) { // Need owner, mfg, retail, sn1, pn1, pn2, buyer
            console.error("ERROR: Insufficient signers. This script requires at least 7 signers for all operations.");
            throw new Error(`Insufficient signers available. Need at least 7, but got ${signers.length}.`);
        }
        // Assign roles (7 signers)
        [owner, manufacturer, retailer, sn1, pn1, pn2, buyer] = signers.slice(0, 7);

        // Assign transporter roles (reusing primary/secondary nodes for simplicity)
        const t1 = pn1;
        ...
        try {
            ...
            console.error("ERROR: Contract deployment failed. Likely due to size limits or network issues.", error);
            console.error("Cannot proceed with gas measurements if deployment fails.");
            this.skip(); // Skip subsequent tests if deployment fails
            return; // Exit before block
        }
        ...
        // Add more grantRole calls here if needed and sum gas to results.grantRole_total
        ...
        let setVerifiedNodeGasList = [];
        let setRoleGasList = [];
        let setNodeTypeGasList = [];
        ...
        async function registerNode(addr, isVerified, role, nodeType, name) {
            ...
            if (isVerified !== null) {
                ...
                nodeRegGas += gas;
                results.setVerifiedNode_total += gas; // Accumulate total
            ...
            if (role !== null) {
                ...
                const gas = BigInt(r.gasUsed);
                nodeRegGas += gas;
                results.setRole_total += gas; // Accumulate total
            ...
            if (nodeType !== null) {
                ...
                const gas = BigInt(r.gasUsed);
                nodeRegGas += gas;
                results.setNodeType_total += gas; // Accumulate total
        ...
        // Removed pn3 registration as only 7 signers are available

        // Register transporters (roles only, assuming verification done above if reused)
        // Only register t1 as transporter for releasePayment test
    ...
    // --- Marketplace Scenario Test ---
    it("Should measure gas for Marketplace Operations", async function () {
        console.log("\nStarting Marketplace Gas Measurement");
        const price = ethers.parseEther("0.01"); // Example price
        let tx, receipt;
        const t1 = pn1; // Get transporter address

        // Ensure NFT is owned by manufacturer before listing
        expect(await supplyChainNFT.ownerOf(tokenId)).to.equal(manufacturer.address);

        // 1. List Product for Sale (Manufacturer selling)
        ...
        // 2. Initiate Purchase (Buyer initiates)
        console.log(`   Initiating purchase for ${tokenId} by buyer ${buyer.address}`);
        // Use the initial CID stored during setup for verification
        ...
        // 3. Deposit Collateral (Buyer deposits funds, triggers NFT transfer)
        ...
        console.log(`     - depositPurchaseCollateral (includes NFT transfer) Gas: ${results.depositCollateral}`);
        expect(await supplyChainNFT.ownerOf(tokenId)).to.equal(buyer.address); // Verify transfer to buyer

        // 4. Release Payment (Buyer confirms receipt and releases funds)
        console.log(`   Releasing payment for ${tokenId} by buyer ${buyer.address}`);
        // Note: releasePurchasePayment requires the buyer to be msg.sender and NFT owner
        // It handles transferring the purchase.price to the seller internally.
        tx = await supplyChainNFT.connect(buyer).releasePurchasePayment(tokenId, t1.address, true);
        receipt = await tx.wait(1);
    ...
    after(async function () {
        // Check if results have been populated (i.e., if tests ran)
        if (results.minting === BigInt(0) && results.sellProduct === BigInt(0)) {
            console.log("\nSkipping final report as tests did not run (likely due to deployment failure).");
            return;
        }
        ...
        console.log("* Note: Ensure you have at least 7 signers available in your Hardhat/network configuration.");
        console.log("* Note: Add these gas costs to your existing transport scenario costs manually.");
    });
});
