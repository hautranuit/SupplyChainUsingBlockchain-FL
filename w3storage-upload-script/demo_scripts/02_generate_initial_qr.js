...
async function main() {
    ...
    if (args.length < 1) {
        console.error("❌ Usage: node 02_generate_initial_qr.js <tokenId>");
        process.exit(1);
    }
    ...
    console.log(`Fetching IPFS CID for Token ID: ${tokenId} from contract ${CONTRACT_ADDRESS}...`);
    ...
    try {
        ...
        if (!ipfsCID || ipfsCID.trim() === "") {
            console.error(`❌ No IPFS CID found for Token ID: ${tokenId}. Ensure the backendListener has processed the ProductMinted event and stored the initial CID.`);
            process.exit(1);
        }
        ...
        const qrFilePath = path.join(qrOutputDir, `token_${tokenId}_initial_qr_${Date.now()}.png`);
        cleanupOldQRCodes(tokenId);
        await generateQRCodeToFile(finalPayload, qrFilePath);

        console.log("\n✅ Initial QR Code generated successfully.");
        console.log(`   QR Image Path: ${qrFilePath}`);
        console.log(`   Encrypted Payload for next step: ${finalPayload}`);

    } catch (error) {
        console.error("❌ Error generating initial QR code:", error.message);
